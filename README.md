...
Suppose we want to draw sample from p-dimensioanl normal $`X_{p\times 1} \sim N(\mu_{p\times 1}, \Sigma_{p\times p})`$, and similarly for Student's t with df=$`\nu`$,

subject to constranst(s): $`l\leq DX \leq u`$,
where  $`D`$ is $`m\times m`$, even if $m > p$.
